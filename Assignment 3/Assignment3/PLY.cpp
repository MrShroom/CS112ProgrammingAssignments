...
void PLYObject::draw()
{
	...
	else
	{
		...
		Vector3f V, L, H, amb = { 0, 0, 0 }, dif = { 0, 0, 0 }, spe = { 0, 0, 0 }, total = { 0, 0, 0 };
		GLfloat k1[1] = { 3}, k2[1] = { 0 }, k3[1] = { 0 }, d, att;

		//V is the unit vector towards the eye
		//L in the unit Vector towards the light
		//H = (L+V)/|L+V|
		//Vertex color = (material emission) + (global ambient)
		//*(material ambient) + Sigma(1 / (k1 + k2*d + k3*d ^ 2)) 
		//*[(light ambient)*(material ambient) + (max{ L.N, 0 })*(light diffuse)
		//*(material diffuse) + (max{ H.N, 0 }) ^ n *(light specular)*(material specular)]

		glDisable(GL_LIGHTING);
		for (int v = 0; v < nv; v++)
		{
			//(material emission) = 0 b/c no material emission
			//(global ambient)*(material ambient) = 0 b/c no global abient
			//assume only one light

			//amb=(light ambient)*(material ambient)
			multVectors(amb, ambient, ambient_light);
			

			//diff = (max{ L.N, 0 })*(light diffuse)*(material diffuse)
			sub(L, light_pos, vertices[v]);
			normalize(L);
			multVectors(dif, diffuse, light_color);
			GLfloat s = dotProd(L, normals[v]);
			scale(dif, max(s , 0));

			//spe = (max{ H.N, 0 }) ^ n *(light specular)*(material specular)
			sub(V, viewer_pos, vertices[v]);
			normalize(V);
			add(H, L, V);
			normalize(H);

			multVectors(spe, light_color, specular);
			s = dotProd(H, normals[v]);
			...
			//att = 1 / (k1 + k2*d + k3*d ^ 2)
			d = sqrtf(L[0] * L[0] + L[1] * L[1] + L[2] * L[2]);
			att = 1/ (k1[0] + k2[0] *d + k3[0] * d*d);
			scale(total, att);
