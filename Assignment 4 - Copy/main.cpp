...
char *fragmentFileName = "fraggeometry.txt";
...
//#3 Texture Mapping with out shading 
//char *vertexFileName = "vertmapping.txt";
//char *fragmentFileName = "fragmapping.txt";

//#3 Texture Mapping with shading 
//char *vertexFileName = "vertmappingWithShading.txt";
//char *fragmentFileName = "fragmappingWithShading.txt";
...
int frame = 0, time, timebase = 0, timebase2 = 0;
char s[50];
...
void setUniforms() {
	...
	if (texID == -1)
		texID = loadBMP_custom(myTextureBmp);
...
void myMulti(float *vecThree, float * matrix)
{
	float temp[] = { vecThree[0], vecThree[1], vecThree[2] };
	vecThree[0] = (temp[0] * matrix[0] + temp[1] * matrix[1] + temp[2] * matrix[2]);
	vecThree[1] = (temp[0] * matrix[4] + temp[1] * matrix[5] + temp[2] * matrix[6]);
	vecThree[2] = (temp[0] * matrix[8] + temp[1] * matrix[9] + temp[2] * matrix[10]);
}
...
void renderScene(void) {
    ...
    glBindVertexArray(vert[0]);
	glDrawArrays(GL_POINTS, 0, sizeof(vertices1));
...
GLuint initShaders() {
	...
	ambientColorLoc = glGetUniformLocation(p, "ambientColor");
...
void mouseMove(int x, int y) 
{
  ...
  case 1:
    /* rotating the view*/
	angle = angle + (x - startX)/1.5;
    angle2 = angle2 + (y - startY)/1.5;
    startX = x;
    startY = y;
    break;
